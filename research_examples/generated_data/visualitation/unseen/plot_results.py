...
for case in cases:
	...
	error = abs(np.array([beta[2] for beta in betas_final]) - np.array(coefs_a2))
...
for case in cases:
	betas_final = [encyclopedia[a]['betas_'+case] for a in range(n_range)]
	error = abs(np.array([beta[1] for beta in betas_final]) - np.array(coefs_a1))
...
#	rel_error = abs(error/coefs_a2)
	error_a1 =  abs(np.array([beta[1] for beta in betas_final]) - np.array(coefs_a1))
	rel_error_a1 = abs(error_a1/coefs_a1)
	error_a2 = abs(np.array([beta[2] for beta in betas_final]) - np.array(coefs_a2))
