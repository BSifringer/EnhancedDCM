...
model_inputs, train_labels = get_inputs_labels(path, 'generated', '_train', lmnlArchitecture=True)
model_test_inputs, test_labels = get_inputs_labels(path, 'generated', '_test', lmnlArchitecture=True)

model = fetch_model("FreezeB", path, extend)
...
model = fetch_model("freezeNN", path, extend)
betas = ghu.get_betas(model)
