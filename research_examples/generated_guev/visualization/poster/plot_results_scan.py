...
neurons = [2, 5, 10, 15, 25, 50, 100, 200, 500, 1001, 2000]

# For data-reading purposes on logscale, MNL neuron 0, is plotted as a neuron 1.
neurons = [1,2, 5, 10, 15, 25, 50, 100, 200, 500, 1001, 2000, 5000]
...
for i in range(n_betas-1):
	...
	betas_i = np.abs(np.array([element[i] for element in beta_list]))
	ax1.semilogx(np.array(neurons), betas_i.flatten(), '.-', label = r"$|\hat\beta_{}$|".format(subscript[i]))

ax1.semilogx(np.array(neurons), np.ones(len(neurons))*2, '--', color = colors[0], label = r"$|\beta_{p, true}|$")
ax1.semilogx(np.array(neurons), np.ones(len(neurons))*1, '--', color = colors[1], label = r"$|\beta_{a, true}|$")
