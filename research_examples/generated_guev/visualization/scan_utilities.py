...
def scan_iteration(number, cases, paths_extends, filePath, fileInputBase, encyclopedia, std_flag=False):
	...
	def get_model_inputs_labels(filePart):
		...
		model_inputs['HRUSHCKA_MNL_HYP'] = [np.expand_dims(inputs_simple[:,:,i],-1) for i in range(inputs_simple.shape[2])]
		model_inputs['HRUSHCKA_FULL_MNL_HYP'] = [np.expand_dims(inputs[:,:,i],-1) for i in range(inputs.shape[2])]
		model_inputs['HRUSHCKA_FULL_MNL_HYP_100'] = [np.expand_dims(inputs[:,:,i],-1) for i in range(inputs.shape[2])]
